...
class UpdatePayPasswordController: SNBaseViewController {
    ...
    fileprivate var cell:UpdatePayCell = UpdatePayCell()
    
    fileprivate let tableView:UITableView = UITableView().then{
        $0.backgroundColor = color_bg_gray_f5
        $0.register(UpdatePayCell.self)
    ...
    @objc func submitAction(){
        ...
        if cell.pwd.textfield.text! == "" {
            SZHUD("请填写新6位数字密码", type: .info, callBack: nil)
            return
        }
...
extension UpdatePayPasswordController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:UpdatePayCell = tableView.dequeueReusableCell(forIndexPath: indexPath)
