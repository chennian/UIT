...
class UpdatePwdCell:SNBaseTableViewCell {
    ...
    override func setupView() {
        ...
        baseView.addSubviews(views: [phone,oldPwd,pwd,confirmPwd,code,submitButton]);
        phone.set(name: "手机号", placeHolder: "请输入手机号码")
        oldPwd.set(name: "旧密码", placeHolder: "请输入旧密码")
        ...
        pwd.snp.makeConstraints { (make) in
            make.top.equalTo(oldPwd.snp.bottom)
            make.left.equalToSuperview().snOffset(35)
            make.right.equalToSuperview().snOffset(-35)
            make.height.snEqualTo(110)
        }
