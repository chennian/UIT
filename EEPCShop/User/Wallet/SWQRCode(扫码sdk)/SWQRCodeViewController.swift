...
extension SWQRCodeViewController {
    ...
    func sw_handle(value: String) {
        ...
        if value.hasPrefix("UITOKEN://") {
            
            let start = value.index(value.startIndex, offsetBy: 10)
            let end = value.index(value.startIndex, offsetBy: value.count)
            
            let uid = String(value[start..<end])
            let vc = PayWebViewController()
            vc.shop_id = uid
            vc.uid = XKeyChain.get(UITOKEN_UID)
            self.navigationController?.pushViewController(vc, animated: true)
        }else{
            SZHUD("二维码无效", type: .info, callBack: nil)
